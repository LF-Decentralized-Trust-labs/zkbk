...
impl ConstraintSynthesizer<ConstraintF> for IssuerCircuit {
    fn generate_constraints(
        ...
        >(
            ...
            &utils::serialize(&user_record_com.x)
        );
...
fn setup_witness() -> IssuerCircuit {
    ...
    let message = utils::serialize(&user_record.commitment().into_affine().x);
    ...
    let statement_digest_encoding = RO::evaluate(&(), &("Hart is amazing!").as_bytes()).unwrap();
    let statement_digest = ConstraintF::from(
        BigInt::<6>::from_bits_le(utils::bytes_to_bits(&statement_digest_encoding).as_slice())
    );
